...
public class Main {
    public static void main(String[] args) {
        ...
        do {
            switch (animalChoiceMenu(keyboard)) {
                 ...
                 */
                case 5:
                    // Display the saved animals from files
                    readObjectsFromFile();
                    break;
    ...
    public static void writeObjectsToFile(Tiger tiger, Penguin penguin, Dolphin dolphin) {
        try (ObjectOutputStream tigerStream = new ObjectOutputStream(new FileOutputStream("tiger.ser"));
             ObjectOutputStream penguinStream = new ObjectOutputStream(new FileOutputStream("penguin.ser"));
             ObjectOutputStream dolphinStream = new ObjectOutputStream(new FileOutputStream("dolphin.ser"))) {

            // Serialize objects
            tigerStream.writeObject(tiger);
            penguinStream.writeObject(penguin);
            dolphinStream.writeObject(dolphin);

            System.out.println("Animal objects saved successfully.");
        } catch (IOException e) {
    ...
    private static void readObjectsFromFile() {
        try {
            Tiger tiger = (Tiger) readAnimalStateFromFile("tiger.ser");
            Penguin penguin = (Penguin) readAnimalStateFromFile("penguin.ser");
            Dolphin dolphin = (Dolphin) readAnimalStateFromFile("dolphin.ser");

            // Print the state of each animal
            System.out.println("----- Tiger State -----");
            printAnimalState(tiger);

            System.out.println("----- Penguin State -----");
            printAnimalState(penguin);

            System.out.println("----- Dolphin State -----");
            printAnimalState(dolphin);
    ...
    private static Animal readAnimalStateFromFile(String fileName) throws IOException, ClassNotFoundException {
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(fileName))) {
            return (Animal) in.readObject();
        }
    }

    private static void printAnimalState(Animal animal) {
        System.out.println("Name: " + animal.getNameOfAnimal());
        ...
        if (animal instanceof Tiger) {
            ...
            System.out.println("Is Swimming: " + penguin.isSwimming());
